...
RSpec.describe "shops#show", type: :request do
  ...
  describe "#filter" do
    ...
    let(:filter_users) { json_body.dig(:data, :relationships, :users, :links, :related) }
    ...
    it 'return cards associated with shop' do
      fetch filter_cards
    ...
    it 'return users associated with shop through cards' do
      fetch filter_users

      aggregate_failures "testing response" do
        expect(response).to have_http_status(:ok)
        expect(json_body[:data]).to have_attributes(size: 1)
        expect(json_body[:data].first).to have_id(user[:id])
        expect(json_body[:data].first).to have_type(:users)
        expect(json_body[:data].first).to have_jsonapi_attributes(:email, :negative_balance).exactly
        expect(json_body[:data].first).to have_relationships(:cards, :shops).exactly
        expect(json_body[:data].first.dig(:relationships, :cards)).to have_link(:related).with_value("/api/v1/cards?filter[user_id]=1")
        expect(json_body[:data].first.dig(:relationships, :shops)).to have_link(:related).with_value("/api/v1/shops?filter[user_id]=1")
        expect(json_body).to have_meta
      end
    end
  end
